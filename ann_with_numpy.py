...
plt.show
# %% MODIFICATION FOR TRAINING AND VALIDATION VARIANT
# reset the network:
network = initialize_network(input_size, hidden_size, output_size)
...
def create_mini_batches(X, Y, batch_size):
    ...
    if data_size % batch_size != 0:
        # Behandelt den letzten Batch, der kleiner sein könnte als batch_size
        excerpt = indices[data_size - data_size % batch_size:data_size]
        X_mini = X[excerpt]
        Y_mini = Y[excerpt]
        mini_batches.append((X_mini, Y_mini))
    return mini_batches
...
def train_network_mini_batch(network, X, Y, batch_size=64, num_iterations=1000, learning_rate=0.01):
    costs = []  # Speichert den Verlust jeder Iteration
    costs_per_epoch = []  # Speichert den durchschnittlichen Verlust jeder Epoche
    num_mini_batches = len(X) // batch_size  # Bestimmt die Anzahl der Mini-Batches pro Epoche

    for i in range(num_iterations):
        mini_batches = create_mini_batches(X, Y, batch_size)
        epoch_costs = 0  # Initialisiert den Verlust der aktuellen Epoche

        for mini_batch in mini_batches:
            (X_mini, Y_mini) = mini_batch
            activations = feedforward(network, X_mini)
            cost = compute_cost(activations['a2'], Y_mini)
            costs.append(cost)  # Speichert den Verlust des Mini-Batches
            epoch_costs += cost  # Summiert den Verlust aller Mini-Batches der Epoche
            gradients = backpropagate(network, activations, X_mini, Y_mini)
            update_parameters(network, gradients, learning_rate)

        average_epoch_cost = epoch_costs / num_mini_batches  # Berechnet den Durchschnittsverlust der Epoche
        costs_per_epoch.append(average_epoch_cost)  # Speichert den Durchschnittsverlust der Epoche

        # Optional: Druckt den Durchschnittsverlust jeder Epoche
        if (i + 1) % num_mini_batches == 0:
            print(f"Kosten nach Epoche {(i + 1) // (num_iterations // num_mini_batches)}: {average_epoch_cost}")

    return costs, costs_per_epoch
...
batch_size = 32
learning_rate = 0.5
loss_history, loss_per_epoch = train_network_mini_batch(network, X_train_flat, y_train_one_hot, 
                             num_iterations=iterations, learning_rate=learning_rate, batch_size=batch_size)

# calculate the epochs:
mini_batch_per_epoch = len(X_train_flat) / batch_size
epochs = iterations / mini_batch_per_epoch
#epochs = iterations // (len(X_train_flat) / batch_size)
print(f'Epochs: {epochs}')

# Testdaten vorhersagen
predictions = predict(network, X_test_flat)

# Die tatsächlichen Labels aus den "one-hot"-Vektoren extrahieren
actual_labels = y_test # np.argmax(y_test, axis=1)

# Genauigkeit berechnen
accuracy = np.mean(predictions == actual_labels)
print(f'Genauigkeit des Netzwerks auf den Testdaten: {accuracy * 100:.2f}%')
